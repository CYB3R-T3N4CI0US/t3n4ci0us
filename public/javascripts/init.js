  ...
  "use strict";
	...
	var Frenifytas = {
		...
		activeNavigation: function(){
			$('.nav_overlay,.tas_fn_nav').addClass('active');	
		},
		...
		vegasSlider: function(){
			$('.tas_fn_hero .vegas-slide').each(function(){
				var element = $(this);
				var html	= []; 
		...
		heroJarallaxSlider: function(){
			// slider
			var section		= $('.tas_fn_hero .swiper-container');
		...
		totop: function (){
			var minSpeed 		= 500;
			var maxSpeed		= 1500;
			$(".tas_fn_totop").off().on('click', function(e) {
				e.preventDefault();
		...
		moreCategories: function(){
			$('.tas_fn_categories').each(function(){
				var element 	= $(this);
		...
		showMore: function(){
			$('.tas_fn_categories .more').off().on('click',function(){
				var element		= $(this),
					parent		= element.closest('.tas_fn_categories'),
		...
		productModal: function(){
			var modal = $('.tas_fn_modal.product_modal');
			$('.tas_fn_drops .item a').off().on('click', function(){
				var element		= $(this);
				...
				var wargameURL	= parent.data('modal-wargame-url');
				var discordURL	= parent.data('modal-discord-url');
				var included	= parent.data('included');
				
				if(included === 'on'){
					modal.find('.tas_fn_product_modal').html(parent.find('.hidden_info').html());
				}else{
					
					// attach empty if not setted
					if(!wargameURL){wargameURL = '';}
					if(!discordURL){discordURL = '';}

					// remove disable from modalbox's buttons
					modal.find('.buttons a').removeClass('disable');

					// change wargame URL or disable wargame button
					if(wargameURL === ''){
						modal.find('.wargame').addClass('disable');
					}else{
						modal.find('.wargame').attr('href',wargameURL);
					}

					// change discord URL or disable wargame button
					if(discordURL === ''){
					...
					modal.find('.tas_fn_title .fn_title').text(title);
		...
		applyFilter: function(){
			...
			$('.tas_fn_filters .checkbox').off().on('click',function(){
				...
				var element 	= $(this),
					...
					// collection wrapper
					parent		= element.closest('.tas_fn_collection'),
					
					// filter result box
					resultBox	= parent.find('.tas_fn_result_box'),
				...
				// if clicked item has already clicked and clicked second time
					...
					var index = tasFilterArray.indexOf(id);
					if(index !== -1){
		...
		recallGridAfterFiltering: function(clear){
			...
			if(tasFilterArray.length === 0){
				filters = '*';
			}
			// if there has one or more selected filters
			else{
				$.each(tasFilterArray, function(index,value){
					filters += '.id'+value;
				});
			}
		...
		removeFilter: function(){
			$('.tas_fn_result_box .result_item a').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.tas_fn_collection'),
					resultItem	= e.closest('.result_item'),
					resultBox	= parent.find('.tas_fn_result_box'),
					id 			= resultItem.data('id'),
					filterCount = resultBox.find('.filter_count span');
				resultItem.remove();
				parent.find('.tas_fn_filters .checkbox[data-id="'+id+'"]').removeClass('selected').find('input[type="checkbox"]').prop('checked','');
				filterCount.text(parseInt(filterCount.text())-1);
				if(resultBox.find('.result_item').length === 0){
					resultBox.find('.clear_all').remove();
				}
			...
			$('.tas_fn_result_box .clear_all').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.tas_fn_collection'),
					filterBox	= parent.find('.tas_fn_filters'),
					resultBox	= parent.find('.tas_fn_result_box'),
					filterCount = resultBox.find('.filter_count span');
				filterCount.text(0);
		...
		movingTextForNav: function(){
			...
			if(overlay.length){
				if(!$('.tas_fn_moving_text').length){
					$('body').append('<div class="tas_fn_moving_text">Close</div>');
				}
				var movingBox		= $('.tas_fn_moving_text');
		...
		navFixer: function(){
			var footerHeight = $('.tas_fn_nav .nav_footer').outerHeight();
			$('.tas_fn_nav .nav_content').css({height: ($(window).height() - footerHeight) + 'px' });
		},
		
		movingBlog: function(){
			var blog = $('.tas_fn_moving_blog');
			if(blog.length){
				if(!$('.tas_fn_moving_box').length){
					$('body').append('<div class="tas_fn_moving_box">');
				}
				var movingBox		= $('.tas_fn_moving_box');
				var list			= $('.tas_fn_moving_blog .item');	
		...
		timeLine: function(){
			var allContentItems 	= $('.tas_fn_timeline .timeline_item');
			var progressLine		= $('.tas_fn_timeline .progress_line');
			var allProgressItems 	= $('.tas_fn_timeline .progress_line li');
			var activeProgressItem 	= $('.tas_fn_timeline .progress_line .active');
			var activeLine 			= $('.tas_fn_timeline .progress_line .active_line');
			var allProgressButtons	= $('.tas_fn_timeline .progress_line a');
			...
			var nextActive = true;
			var prevActive = true;
			var prevButton = $('.tas_fn_timeline .nav_prev');
			var nextButton = $('.tas_fn_timeline .nav_next');
		...
		timelineClasses: function(allContentItems,allProgressItems,activeIndex){
			...
			isActive 	= false;
			if(activeIndex === 'initial'){
				extraActiveIndex = $('.tas_fn_timeline .progress_line .active').data('index');
			}else{
		...
		headerTrigger: function(){
			var trigger 		= $('.tas_fn_header .trigger');
			var navOverlay		= $('.nav_overlay');
			var rightNav		= $('.tas_fn_nav');
			var trigger2 		= rightNav.find('.trigger');
			var links			= $('.tas_fn_nav .nav_menu a');
			var buttons			= $('.tas_fn_nav .nav_buttons');
			var footer			= $('.tas_fn_nav .nav_footer');

			var menuItems		= $('.tas_fn_nav .nav_menu > ul > li');
